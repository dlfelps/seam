...
- `./.seam-work/iteration.txt` — current iteration number (1, 2, or 3).
- `./.seam-work/critic-feedback.md` — present only on iterations 2 and 3. The first line is `APPROVE` or `REJECT`. If `REJECT`, every coupling concern listed must be addressed in this iteration.
- `./.seam-work/interfaces/` — contains your previous draft on iterations 2 and 3. Revise these files; do not start from scratch unless the Critic's feedback indicates the whole structure is wrong.
...
3. On iterations 2+, address every concern in the Critic's REJECT report. If the Critic said a feature would require modifying interface X, restructure X so the feature can be added as a new implementation instead.
